...
def get_abs_path(f_type: str) -> str:
    return f"{path.dirname(__file__)}{sep}files{sep}{f_type}{sep}"


window = Tk()
window.iconbitmap(f'{get_abs_path("pict")}iconfortest.ico')
window.state('zoomed')
...
class MainMenu:
    def __init__(self):
        ...
        img = PhotoImage(file=f"{get_abs_path('pict')}zadnifon.png")
    ...
    def exit_app(self):
        ...
        #confirm_window.iconbitmap(f"{get_abs_path('pict')}iconfortest.ico")
    ...
    def start_theme(self, theme):
        theme_mapping = {
            "Unit_1": f"{get_abs_path('pdf')}objie_polojenia.pdf",
            "Unit_2": f"{get_abs_path('pdf')}objie_obazannosti.pdf",
            "Unit_3": f"{get_abs_path('pdf')}primenenie_spec_signalov.pdf",
        }
...
class BilTest:
    ...
    def show_bil(self):
        ...
        Button(self.canvas,
               text=f"Билет 1",
               command=lambda: self.start_bil(f'{get_abs_path("text")}bil_0.txt')).place(x=x, y=20 + self.padding * 0)
        Button(self.canvas,
               text=f"Билет 2",
               command=lambda: self.start_bil(f'{get_abs_path("text")}bil_1.txt')).place(x=x, y=20 + self.padding * 1)
        Button(self.canvas,
               text=f"Билет 3",
               command=lambda: self.start_bil(f'{get_abs_path("text")}bil_2.txt')).place(x=x, y=20 + self.padding * 2)
        #Button(self.canvas,
               #text=f"Билет 4",
               #command=lambda: self.start_bil(f'{get_abs_path("text")}bil_3.txt')).place(x=x, y=20 + self.padding * 3)
        #Button(self.canvas,
               #text=f"Билет 5",
               #command=lambda: self.start_bil(f'{get_abs_path("text")}bil_4.txt')).place(x=x, y=20 + self.padding * 4)
...
class TeachBook:
    def __init__(self, theme, pdf_file):
        ...
        Label(self.canvas, text="Для перелистывания страниц используйте клавиши ← и → на вашей клавиатуре",bg='#A2A2D0',
              font=("Times new Roman", 20)).place(x=500, y=900)

        self.button = tk.Button(self.canvas, text="Тестирование по данной теме", command=self.start_test)
...
theme_dict = {
    "Unit_1": f"{get_abs_path('text')}unit_1.txt",
    "Unit_2": f"{get_abs_path('text')}unit_2.txt",
    "Unit_3": f"{get_abs_path('text')}unit_3.txt",
}
...
class Tester:
    ...
    def load_questions(self, filename):
        ...
        with open(filename, "r") as file:
            ...
            for i in range(0, len(lines), 6):
                question = {
                    "image": f'{get_abs_path("pict")}{lines[i].strip()}',
    ...
    def exit_bilet(self):
        confirm_window = Toplevel(window)
        confirm_window.grab_set()
        confirm_window.geometry("500x250+1200+400")
        #confirm_window.iconbitmap(f'{get_abs_path("pict")}iconfortest.ico')
        confirm_window.resizable(width=False, height=False)
        confirm_window.title("Выход из приложения")
        Label(confirm_window, text="Вы уверенны, что хотите выйти?", font=("Times new Roman", 20)).place(x=50, y=10)
    ...
    def show_question(self):
        ...
        if self.current_question < len(self.questions):
            ...
            self.canvas.create_image(950, 300, image=self.image)
